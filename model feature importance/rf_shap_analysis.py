...
OUT_DIR = "../model feature importance/rf_shap_analysis"
...
def load_rf_params(csv_path):
    ...
    required_cols = ["horizon", "rf_val_max_features", "n_estimators"]
    ...
    for _, row in df.iterrows():
        ...
        param_map[target] = {
            ...
            "max_features": float(row["rf_val_max_features"])   # tuned float fraction from RandomizedSearchCV
        }
...
# -----------------------------------------------------------------------
def compute_shap_for_h(full_df, xt_cols, h, params):   # FIX 4: removed unused all_features parameter; flat_names always derived locally from xt_cols
    """
...
for h in HORIZONS:
    ...
    base_shap     = compute_shap_for_h(full_df, xt_cols, h, rf_params[f"y_h{h}"])   # FIX 4: removed all_features argument
    shap_by_h[h]  = base_shap

    # Save raw SHAP importance to CSV
